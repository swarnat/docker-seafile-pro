...
INSTALLPATH=${INSTALLPATH:-"${BASEPATH}/$(ls -1 ${BASEPATH} | grep -E '^seafile-pro-server-[0-9.-]+')"}
VERSION=$(echo $INSTALLPATH | grep -oE [0-9.]+)

export SERVER_IP=${SEAFILE_ADDRESS:-"127.0.0.1"}

if test -f "$DATADIR/version"; 
then
...
choose_setup() {
  set +u
  # If $MYSQL_HOST is set, we assume MYSQL setup is intended,
  # otherwise sqlite
  if [ -n "${MYSQL_HOST:-}" ]
  then
    set -u
...
setup_mysql() {
  ...
  dockerize -timeout ${DOCKERIZE_TIMEOUT} -wait tcp://${MYSQL_HOST}:${MYSQL_PORT:-3306}
